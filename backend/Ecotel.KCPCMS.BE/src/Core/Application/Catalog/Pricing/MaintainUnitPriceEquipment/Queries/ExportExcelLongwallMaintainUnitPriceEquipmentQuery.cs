...
public record ExportExcelLongwallMaintainUnitPriceEquipmentQuery() : IRequest<byte[]>;

public class ExportExcelLongwallMaintainUnitPriceEquipmentQueryHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<ExportExcelLongwallMaintainUnitPriceEquipmentQuery, byte[]>
{
    ...
    public async Task<byte[]> Handle(ExportExcelLongwallMaintainUnitPriceEquipmentQuery request, CancellationToken cancellationToken)
    {
        ...
        var filteredEquipmentIds = equipments
            .Where(e => e.EquipmentProcessGroups.Any(epg => epg.ProcessGroup != null && epg.ProcessGroup.Type == ProcessGroupType.LC))
            .Select(e => e.Id)
            .ToHashSet();
