...
public record ExportExcelTrimmingMaintainUnitPriceEquipmentQuery() : IRequest<byte[]>;

public class ExportExcelTrimmingMaintainUnitPriceEquipmentQueryHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<ExportExcelTrimmingMaintainUnitPriceEquipmentQuery, byte[]>
{
    ...
    public async Task<byte[]> Handle(ExportExcelTrimmingMaintainUnitPriceEquipmentQuery request, CancellationToken cancellationToken)
    {
