...
public class ImportTrimmingMaintainUnitPriceEquipmentExcelCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<ImportTrimmingMaintainUnitPriceEquipmentExcelCommand, bool>
{
    ...
    public async Task<bool> Handle(ImportTrimmingMaintainUnitPriceEquipmentExcelCommand request, CancellationToken cancellationToken)
    {
        ...
        var equipmentIdMap = equipments
            .Where(e => e.Code != null
                && e.EquipmentProcessGroups.Any(epg => epg.ProcessGroup != null && epg.ProcessGroup.Type == ProcessGroupType.XL))
    ...
    private List<TrimmingMaintainUnitPriceImportDto> ParseTransposedExcel(Stream stream)
    {
    ...
    private async Task<bool> CheckExistedReferences(List<TrimmingMaintainUnitPriceImportDto> dtoList)
    {
