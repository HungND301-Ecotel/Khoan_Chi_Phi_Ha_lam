...
public class ImportTunnelSupportAndDrillingMaterialUnitPriceExcelCommandHandler(IUnitOfWork unitOfWork, ICacheService cacheService) : IRequestHandler<ImportTunnelSupportAndDrillingMaterialUnitPriceExcelCommand, bool>
{
        ...
        string endMonth)
    {
            ...
            string materialDisplay)
        {
            ...
            {
                throw new BadRequestException(
                    $"Cặp Nhóm vật tư, tài sản '{assignmentDisplay}' và Vật tư tài sản '{materialDisplay}' bị trùng cho mã định mức vật liệu '{Code}'.");
            }

            _assignmentCostMap[key] = new AssignmentCostImportItem(
                assignmentCodeId,
                materialId,
                norm);
        }
