...
public class ImportLongwallMaterialUnitPriceExcelCommandHandler(IUnitOfWork unitOfWork, ICacheService cacheService)
    : IRequestHandler<ImportLongwallMaterialUnitPriceExcelCommand, bool>
{
    ...
    private readonly IWriteRepository<LongwallMaterialUnitPriceEntity> _materialUnitPriceRepository =
        unitOfWork.GetRepository<LongwallMaterialUnitPriceEntity>();
    private readonly IWriteRepository<ProductionProcess> _processRepository =
        unitOfWork.GetRepository<ProductionProcess>();
    private readonly IWriteRepository<LongwallParameters> _longwallParametersRepository =
        unitOfWork.GetRepository<LongwallParameters>();
    private readonly IWriteRepository<CuttingThickness> _cuttingThicknessRepository =
        unitOfWork.GetRepository<CuttingThickness>();
    private readonly IWriteRepository<SeamFace> _seamFaceRepository =
        unitOfWork.GetRepository<SeamFace>();
    private readonly IWriteRepository<Technology> _technologyRepository =
        unitOfWork.GetRepository<Technology>();
    private readonly IWriteRepository<Hardness> _hardnessRepository =
        unitOfWork.GetRepository<Hardness>();
    private readonly IWriteRepository<Power> _powerRepository =
        unitOfWork.GetRepository<Power>();
    private readonly IWriteRepository<AssignmentCode> _assignmentCodeRepository =
        unitOfWork.GetRepository<AssignmentCode>();
    private readonly IWriteRepository<MaterialUnitPriceAssignmentCode> _materialUnitPriceAssignmentCodeRepository =
        unitOfWork.GetRepository<MaterialUnitPriceAssignmentCode>();
    private readonly IWriteRepository<Material> _materialRepository =
        unitOfWork.GetRepository<Material>();
    private readonly IWriteRepository<Cost> _costRepository =
        unitOfWork.GetRepository<Cost>();
    ...
    public async Task<bool> Handle(ImportLongwallMaterialUnitPriceExcelCommand request, CancellationToken cancellationToken)
    {
        ...
        // --- ĐỌC THÁNG TỪ FILE TRƯỚC ĐỂ LOAD COST ĐÚNG KỲ ---
        DateOnly startMonthDate;
        using (var peekStream = request.File.OpenReadStream())
        {
            using var wb = new XLWorkbook(peekStream);
            var ws = wb.Worksheet(1);
            var startMonthStr = ws.Cell(3, 2).GetString().Trim();
            startMonthDate = ParseMonthYear(string.IsNullOrWhiteSpace(startMonthStr)
                ? new DateOnly(DateTime.Now.Year, DateTime.Now.Month, 1).ToString("MM/yyyy")
                : startMonthStr);
        }
        ...
        var materials = await _materialRepository.GetAllAsync(
            include: m => m.Include(x => x.Code)
                           .Include(x => x.AssignmentCode)
                               .ThenInclude(a => a.Code),
            disableTracking: true);

        // Load đơn giá vật tư theo kỳ tháng
        var allMaterialCosts = await _costRepository.GetAllAsync(
            predicate: c => c.CostType == CostType.Material &&
                            c.MaterialId != null &&
                            c.StartMonth <= startMonthDate &&
                            c.EndMonth >= startMonthDate,
            disableTracking: true);
        ...
        var importRows = ParseFromCustomTemplate(stream, assignmentLookup, costByMaterialId, importErrors);
        ...
        foreach (var row in importRows)
        {
            ...
            try
            {
                ...
                if (dbCodeLookup.TryGetValue(code, out var existing))
                {
                    ...
                    existing.Update(code, processId, longwallParametersId, cuttingThicknessId, seamFaceId,
                        powerId, hardnessId, powerId.HasValue, technologyId, startMonth, endMonth,
                        row.OtherMaterialValue, costs);
                    updateList.Add(existing);
                    matchedCodes.Add(code);
                }
                else
                {
                    var newEntity = LongwallMaterialUnitPriceEntity.Create(code, processId, longwallParametersId,
                        cuttingThicknessId, seamFaceId, powerId, hardnessId, powerId.HasValue, technologyId,
                        startMonth, endMonth, row.OtherMaterialValue, costs);
    ...
    private static List<ParsedLongwallMaterialUnitPriceRow> ParseFromCustomTemplate(
        ...
        IReadOnlyDictionary<Guid, double> costByMaterialId,
        ICollection<string> importErrors)
    {
        ...
        for (int col = 7; col <= lastCol; col++)
        {
            ...
            {
                var materialName = !string.IsNullOrWhiteSpace(row6Val) ? row6Val : row5Val;
                ...
                foreach (var code in codes)
                {
                    ...
                    if (!aggregates.TryGetValue(key, out var aggregate))
                    {
                        aggregate = new ParsedLongwallMaterialUnitPriceRow(
                            code, processName, technologyName, hardnessName, powerName,
                            longwallParametersName, cuttingThicknessName, block.SeamFaceName,
                            startMonth, endMonth);
                    ...
                    foreach (var mat in block.MaterialCols)
                    {
                        ...
                        if (priceCell.IsEmpty()) continue;

                        if (!TryParseDouble(priceCell, out var norm))
                        {
                            importErrors.Add($"Giá trị không hợp lệ tại dòng {row}, cột {mat.Col} (Mã: {code}).");
                            continue;
                        }

                        if (IsOtherMaterialAssignment(mat.MaterialName))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                importErrors.Add($"Mã vật tư '{mat.MaterialName}' không tồn tại (dòng {row}, cột {mat.Col}).");
                                continue;
                            }
                            ...
                            try
                            {
                                aggregate.AddAssignmentCost(assignment.Id, assignment.MaterialId, norm, totalPrice, assignment.Display);
                            }
    ...
    private static IReadOnlyDictionary<string, AssignmentLookupItem> BuildAssignmentLookup(
        IEnumerable<AssignmentCode> assignments,
        IEnumerable<Material> materials)
    {
        ...
        foreach (var assignment in assignments)
        {
            ...
            var item = new AssignmentLookupItem(assignment.Id, null, display);
        ...
        {
            var matCode = material.Code?.Value?.Trim() ?? string.Empty;
            var matName = material.Name?.Trim() ?? string.Empty;
            var matDisplay = BuildAssignmentDisplay(matCode, matName);

            var matchedAssignment = material.AssignmentCode;
            if (matchedAssignment == null) continue;

            var item = new AssignmentLookupItem(matchedAssignment.Id, material.Id, matDisplay);
            lookup.TryAdd(NormalizeLookupValue(matDisplay), item);
            lookup.TryAdd(NormalizeLookupValue(matCode), item);
        }
        ...
        string code)
    {
        ...
        if (isPowerNone == isHardnessNone)
            throw new BadRequestException($"Mã '{code}': chỉ một trong hai 'Hệ số kiên cố' hoặc 'Công suất' được để trống.");
        ...
        if (!isHardnessNone)
        {
            if (!hardnessIdMap.TryGetValue(normalizedHardness, out var mappedHardnessId))
                throw new BadRequestException($"Hệ số kiên cố '{hardnessName}' không tồn tại cho mã '{code}'.");
    ...
    {
        var deletedIds = deletedEntities.Select(e => e.Id).ToHashSet();
        ...
        var groupedEntities = finalEntities.GroupBy(e => new MonthRangeOverlapKey(
            e.ProcessId, e.TechnologyId, e.LongwallParametersId,
            e.CuttingThicknessId, e.SeamFaceId, e.PowerId, e.HardnessId));

        foreach (var group in groupedEntities)
        {
            var ordered = group.OrderBy(e => e.StartMonth).ThenBy(e => e.EndMonth).ToList();
            var maxEnd = ordered[0].EndMonth;

            for (var i = 1; i < ordered.Count; i++)
            {
                if (maxEnd > ordered[i].StartMonth)
                    throw new ConflictException(CustomResponseMessage.MonthRangeOverlap);
                if (ordered[i].EndMonth > maxEnd)
                    maxEnd = ordered[i].EndMonth;
            }
        }
    }
    ...
    private sealed record AssignmentLookupItem(Guid Id, Guid? MaterialId, string Display);

    private sealed record MonthRangeOverlapKey(
        Guid ProcessId, Guid? TechnologyId, Guid LongwallParametersId,
        Guid CuttingThicknessId, Guid SeamFaceId, Guid? PowerId, Guid? HardnessId);

    private sealed record LongwallRowKey(
        string Code, string SeamFaceName, string StartMonth, string EndMonth,
        string ProcessName, string TechnologyName, string HardnessName,
        string PowerName, string LongwallParametersName, string CuttingThicknessName);

    private sealed class ParsedLongwallMaterialUnitPriceRow(
        string code, string processName, string technologyName, string hardnessName,
        string powerName, string longwallParametersName, string cuttingThicknessName,
        string seamFaceName, string startMonth, string endMonth)
    {
        ...
        public void AddAssignmentCost(Guid assignmentCodeId, Guid? materialId, double norm, double totalPrice, string assignmentDisplay)
        {
            if (_assignmentCostMap.ContainsKey(assignmentCodeId))
                throw new BadRequestException($"Nhóm vật tư '{assignmentDisplay}' bị trùng cho mã '{Code}'.");
