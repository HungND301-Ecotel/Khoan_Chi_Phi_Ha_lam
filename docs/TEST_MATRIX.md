...
| `docs/stories/epics/E03-material-catalog/US-001-unify-material-type/` | Material catalog stays unified on one material type, `AssignmentCodeMaterial` distinguishes main vs other material membership through an enum, material readers use `AssignmentCodeIds`, and downstream `Part` vs `OtherPart` cleanup stays deferred | no   | planned     | manual | planned  | changed     | Catalog-foundation work was implemented on 2026-05-26; the current story packet was repaired the same day to use role-aware assignment membership plus multi-group-safe material readers |
