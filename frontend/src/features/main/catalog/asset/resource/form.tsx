...
import {
	...
	AssetResourceFormSchema,
	assetResourceFormSchema,
} from '@/features/main/catalog/asset/resource/schema';
...
export function AssetResourceForm({ data, row }: ActionDialogProps<Asset>) {
	const popup = usePopup();
	const { setOpen } = useDialog();
	const { breadcrumb } = useMeta();
	const [units, setUnits] = useState<Unit[]>([]);

	const form = useForm<AssetResourceFormSchema>({
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				if (row) {
					const res = await api.get<AssetResourceDetail>(
