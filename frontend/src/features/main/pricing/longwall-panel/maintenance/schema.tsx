...
export const longwallCommonFormSchema = z.object({
	equipmentId: z
		.string()
		.nonempty({ error: 'Nhóm vật tư, tài sản không được để trống' }),
});

export type LongwallCommonFormSchema = z.infer<typeof longwallCommonFormSchema>;

export const LONGWALL_COMMON_FORM_DEFAULT: LongwallCommonFormSchema = {
	equipmentId: '',
};

// Aliases for compatibility
export const longwallPanelFormSchema = longwallCommonFormSchema;
export type LongwallPanelFormSchema = LongwallCommonFormSchema;
export const LONGWALL_PANEL_FORM_DEFAULT = LONGWALL_COMMON_FORM_DEFAULT;
	...
	for (let i = 0; i < periods.length; i++) {
		...
		for (let j = 0; j < period.costs.length; j++) {
			...
			if (
				...
				return `${label}: Sản lượng than bình quân tháng (1000 tấn) phải lớn hơn 0.`;
			}
		}
