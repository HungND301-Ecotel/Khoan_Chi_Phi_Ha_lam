...
import type { LongwallPanel } from '@/features/main/pricing/longwall-panel/maintenance/columns';
import type { LongwallPanelDetail } from '@/features/main/pricing/longwall-panel/maintenance/page';
import {
	type SCTXPeriodCost,
	type SCTXPeriodItemData,
	LONGWALL_COMMON_FORM_DEFAULT,
	longwallCommonFormSchema,
	type LongwallCommonFormSchema,
	validateSCTXPeriods,
...
export function LongwallPanelForm({
	...
	const form = useForm<LongwallCommonFormSchema>({
		resolver: zodResolver(longwallCommonFormSchema),
		mode: 'onSubmit',
		defaultValues: {
			...LONGWALL_COMMON_FORM_DEFAULT,
	...
	useEffect(() => {
		...
		api
			.pagging<LinkedMaterial>(API.CATALOG.ASSET.LIST, {
				ignorePagination: true,
				materialType: 1,
			})
			.then((res) => {
	...
	useEffect(() => {
		...
		Promise.all(
			periodsToLoad.map((p) =>
				api.get<LongwallPanelDetail>(
					API.PRICING.MAINTENANCE.LONGWALL_DETAIL(p.id),
				),
			),
		)
			.then((results) => {
				const loadedPeriods: SCTXPeriodItemData[] = results.map(
					(res, index) => {
						...
						const items = (detail?.maintainUnitPriceEquipment || []).filter(
							(c: any) => (c.partType ?? 1) === 1,
						);
	...
	const handleUpdatePeriod = (index: number, updated: SCTXPeriodItemData) => {
		setPeriods((prev) => {
			const next = [...prev];
			next[index] = updated;
			return next;
	...
	const handleSubmit = async (values: LongwallCommonFormSchema) => {
...
interface LongwallSCTXPeriodSectionProps {
...
function LongwallSCTXPeriodSection({
	period,
	totalPeriods,
	equipment,
	parts,
	...
	onDelete,
	onCopy,
}: LongwallSCTXPeriodSectionProps) {
	...
	// Tính tổng chi phí phụ tùng trong kỳ theo công thức Lò chợ:
	// regularRepairCost = (partCost * regularRepairRates) / 1000
		...
		return period.costs.reduce((sum, cost) => {
			...
			const rate =
				...
				cost.averageMonthlyTunnelProduction > 0
					...
					: 0;
			const rowCost =
				((part?.costAmount ?? 0) * rate) / LONGWALL_TON_CONVERSION_FACTOR;
			return sum + rowCost;
		}, 0);
						...
						{period.costs.map((cost) => {
							...
							const rate =
								...
								cost.averageMonthlyTunnelProduction > 0
									...
									: 0;
							const rowCost =
								((part?.costAmount ?? 0) * rate) /
								LONGWALL_TON_CONVERSION_FACTOR;
								...
								<FormRow key={cost.partId} className='min-w-full w-max'>
									...
									<div className='flex min-w-max flex-1 flex-col gap-2'>
										...
										<FormNumberInput
											value={cost.quantity}
											onValueChange={(val) =>
												handleUpdateCostRow(cost.partId, 'quantity', val ?? 0)
											}
