...
import {
	MATERIAL_COMMON_FORM_DEFAULT,
	materialCommonFormSchema,
	type MaterialCommonFormSchema,
...
export function MaterialForm({
	...
	const form = useForm<MaterialCommonFormSchema, unknown, MaterialCommonFormSchema>({
		resolver: zodResolver(materialCommonFormSchema),
		mode: 'onSubmit',
		defaultValues: MATERIAL_COMMON_FORM_DEFAULT,
	});
			...
			rawPeriods.map(async (p, idx) => {
				if (!p.id) {
					return {
						...
						endMonth: (p.endMonth ?? new Date().toISOString()).substring(
							0,
							10,
						),
						costs: [],
				...
				try {
					const res = await api.get<MaterialDetail>(
						API.PRICING.MATERIAL.TUNNELING.DETAIL(p.id),
					);
	...
	const handleSubmit = async (values: MaterialCommonFormSchema) => {
		try {
			...
			for (const period of periods) {
				...
				if (period.id && !isDuplicate) {
					...
					await api.post(API.PRICING.MATERIAL.TUNNELING.CREATE, payload);
				}
			}
