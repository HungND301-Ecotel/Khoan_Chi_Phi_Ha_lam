...
export function SupportAndDrillingForm({
	...
	const watchedStartMonth = useWatch({
		...
		name: 'startMonth',
		defaultValue: SUPPORT_AND_DRILLING_FORM_DEFAULT.startMonth,
	});
	const watchedCosts = useWatch({
		control: form.control,
		name: 'costs',
		defaultValue: SUPPORT_AND_DRILLING_FORM_DEFAULT.costs,
	});
	...
	return (
		<FormProvider context={form} onSubmit={handleSubmit}>
			...
			<GroupedSupportAndDrillingCosts
				...
				<div className='flex flex-col gap-4'>
					...
					{otherMaterialPercent !== undefined ? (
						<div className='flex flex-col gap-4'>
							...
							/>
							<SupportAndDrillingVtkRow materialTotal={materialTotal} />
						</div>
					) : (
...
function OtherMaterialAddButton() {
	const { setValue } = useFormContext<SupportAndDrillingFormSchema>();
...
function SupportAndDrillingVtkRow({
	...
	const { control, setValue } = useFormContext<SupportAndDrillingFormSchema>();
	const otherMaterialPercent = useWatch({
		control,
		name: 'otherMaterialValue',
	});
