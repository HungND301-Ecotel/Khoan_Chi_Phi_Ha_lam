...
export function MainPricingElectricityPage() {
	...
	const handleDelete = async ({ data }: ActionDialogProps<Electricity>) => {
		...
		try {
			await api.delete(
				API.PRICING.ELECTRICITY.TUNNELING.DELETES,
				selectedPeriodIds,
			);
		...
		rows.forEach((row) => {
			...
			const period: ElectricityPeriod = {
				...
				averageMonthlyTunnelProduction:
					row.averageMonthlyTunnelProduction ?? 0,
	...
	return (
		<DataTable
			...
			onCreate={
				hasPermission('pricing.tunnerelectricityunitprice.create')
			...
			onUpdate={
				hasPermission('pricing.tunnerelectricityunitprice.update')
					? (props) => <ElectricityForm {...props} />
					: undefined
			}
			onDelete={
				hasPermission('pricing.tunnerelectricityunitprice.delete')
					? handleDelete
					: undefined
			}
			...
			onExport={
				hasPermission('pricing.tunnerelectricityunitprice.export')
					? handleExport
					: undefined
			}
			onImport={
				hasPermission('pricing.tunnerelectricityunitprice.import')
					? handleImport
					: undefined
			}
			onExpand={(props) => (
														...
														<thead className='border-b border-neutral-200 bg-neutral-100 text-sm font-semibold text-black whitespace-nowrap'>
															<tr>
																...
																<th
																	...
																	className='px-4 py-2.5 text-right font-semibold text-black whitespace-nowrap'
																>
																	Điện năng tiêu thụ 1 thiết bị/1 mét lò đào (kwh/m)
																</th>
																<th
																	scope='col'
																	className='px-4 py-2.5 text-right font-semibold text-black whitespace-nowrap'
