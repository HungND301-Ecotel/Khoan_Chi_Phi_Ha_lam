...
export function MainPricingMaintenanceTunnelingPage() {
	...
	const isRowSelected = (row: Tunneling): boolean | 'indeterminate' => {
	...
	const handleRowSelectChange = (row: Tunneling, checked: boolean) => {
		const periodIds =
	...
	const handleSelectAllRowsChange = (checked: boolean, rows: Tunneling[]) => {
	...
	const transformData = (rows: Tunneling[]): Tunneling[] => {
		const groupMap = new Map<string, Tunneling>();
	...
	return (
		<DataTable
			...
			onCreate={
				hasPermission('pricing.maintainunitprice.create')
			...
			onUpdate={
				hasPermission('pricing.maintainunitprice.update')
					? (props) => <TunnelingForm {...props} />
					: undefined
			}
			onExpand={(props) => (
			...
			onDelete={
				hasPermission('pricing.maintainunitprice.delete')
					? handleDelete
					: undefined
			}
			...
			onExport={
				hasPermission('pricing.maintainunitprice.export')
					? handleExport
					: undefined
			}
			onImport={
				hasPermission('pricing.maintainunitprice.import')
					? handleImport
					: undefined
			}
		/>
		...
		if (!periodDetailsMap[period.id]) {
			...
			try {
				const res = await api.get<TunnelingDetail>(
					API.PRICING.MAINTENANCE.TUNNEL_DETAIL(period.id),
				);
			...
			<table className='w-full text-left text-sm text-black'>
				<thead className='border-b border-neutral-200 bg-neutral-100 text-sm font-semibold text-black'>
					<tr>
						...
						>
							Chi phí vật tư SCTX cho 1 thiết bị/1 mét lò đào (đ/m)
						</th>
						<th
							scope='col'
