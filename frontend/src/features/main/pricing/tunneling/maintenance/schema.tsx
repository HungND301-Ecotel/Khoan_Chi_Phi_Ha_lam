...
export const tunnelingCommonFormSchema = z.object({
	equipmentId: z
		.string()
		.nonempty({ error: 'Nhóm vật tư, tài sản không được để trống' }),
});

export type TunnelingCommonFormSchema = z.infer<typeof tunnelingCommonFormSchema>;

export const TUNNELING_COMMON_FORM_DEFAULT: TunnelingCommonFormSchema = {
	equipmentId: '',
};

// Aliases for compatibility
export const tunnelingFormSchema = tunnelingCommonFormSchema;
export type TunnelingFormSchema = TunnelingCommonFormSchema;
export const TUNNELING_FORM_DEFAULT = TUNNELING_COMMON_FORM_DEFAULT;
	...
	for (let i = 0; i < periods.length; i++) {
		...
		for (let j = 0; j < period.costs.length; j++) {
			...
			if (
				...
				return `${label}: Sản lượng đào lò bình quân phải lớn hơn 0.`;
			}
		}
