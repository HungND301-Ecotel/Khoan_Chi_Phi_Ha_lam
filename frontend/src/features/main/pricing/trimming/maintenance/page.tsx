...
export function MainPricingMaintenanceTrimmingPage() {
	...
	const isRowSelected = (row: Trimming): boolean | 'indeterminate' => {
	...
	const handleRowSelectChange = (row: Trimming, checked: boolean) => {
		const periodIds =
	...
	const handleSelectAllRowsChange = (checked: boolean, rows: Trimming[]) => {
	...
	const transformData = (rows: Trimming[]): Trimming[] => {
		const groupMap = new Map<string, Trimming>();
	...
	return (
		<DataTable
			...
			onCreate={
				hasPermission('pricing.trimmingmaintainunitprice.create')
					? (props) => <TunnelingForm {...props} />
					: undefined
			}
			onDuplicate={
				hasPermission('pricing.trimmingmaintainunitprice.create')
					? (props) => <TunnelingForm {...props} isDuplicate />
					: undefined
			}
			onUpdate={
				hasPermission('pricing.trimmingmaintainunitprice.update')
					? (props) => <TunnelingForm {...props} />
					: undefined
			}
			onExpand={(props) => (
			...
			onDelete={
				hasPermission('pricing.trimmingmaintainunitprice.delete')
					? handleDelete
					: undefined
			}
			...
			onExport={
				hasPermission('pricing.trimmingmaintainunitprice.export')
					? handleExport
					: undefined
			}
			onImport={
				hasPermission('pricing.trimmingmaintainunitprice.import')
					? handleImport
					: undefined
			}
		/>
...
export function TunnelingExpand({
	row,
	selectedPeriodIds,
	onTogglePeriodSelect,
}: ActionDialogProps<Trimming> & {
	...
	const [periodDetailsMap, setPeriodDetailsMap] = useState<
		Record<string, MaintainUnitPriceEquipment[]>
	>({});
	...
	const handleTogglePeriodDetails = async (period: MaintenancePeriod) => {
		...
		if (!periodDetailsMap[period.id]) {
			...
			try {
				const res = await api.get<TunnelingDetail>(
					API.PRICING.MAINTENANCE.TRIMMING_DETAIL(period.id),
				);
				const detail = res.result;
				const items = detail?.maintainUnitPriceEquipment || [];
				const totalPartsCost = items.reduce(
					(sum, item) => sum + (item.materialCostPerMetres || 0),
					0,
				);
			...
			<table className='w-full text-left text-sm text-black'>
				<thead className='border-b border-neutral-200 bg-neutral-100 text-sm font-semibold text-black'>
					<tr>
						...
						>
							Chi phí vật tư SCTX cho 1 thiết bị/1 mét lò xén (đ/m)
						</th>
						<th
							scope='col'
